...
# the profile records, our own git identity fragment, and the
...
sev_doctor() {
  ...
  # 4. the git identity fragment. NOT a sweep of every tool that might consume
  # severance: whoever declared a seam owns checking that their end of it
  # works, and is the only thing that can tell an answer from a failure there.
  # This one is ours because the fragment is ours.
